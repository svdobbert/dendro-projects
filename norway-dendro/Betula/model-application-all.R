...
input <- input[1:(ncol(input)-3)]
input <- input[, !(names(input) %in% c("AMO_winter", "AMO_spring", "AMO_summer", "AMO_autumn", "AMO_summer_preyear", "AMO_autumn_preyear"))]
...
relative_importance_mean <- aggregate(relative_importance, by = list(relative_importance$var, relative_importance$model), function(x) mean(x, na.rm = T))
relative_importance_mean$group <- aggregate(relative_importance, by = list(relative_importance$var, relative_importance$model), function(x) first(x))$group
relative_importance_mean$dataset <- aggregate(relative_importance, by = list(relative_importance$var, relative_importance$model), function(x) first(x))$dataset
relative_importance_mean$var <- relative_importance_mean$Group.1
relative_importance_mean$model <- relative_importance_mean$Group.2

plot_rel_imp <- ggplot(relative_importance_mean) +
  geom_col_pattern(
    aes(
      x = reorder_within(var, rel.inf, model, ),
      y = rel.inf,
...
pdf("relative_importance_mean.pdf", width = 14, height = 5, pointsize = 12)
plot_rel_imp
dev.off()
...
var <- c(var, "AMO_annual", "Evap_era5_winter", "AT_mean_era5_summer")
...
density_selected2 <- density_selected[density_selected$group2 %in% var2, ]
...
partial_dependence <- ggplot(dependencies_selected) +
  ...
  geom_ribbon(data = density_selected2, aes(
    x = x,
...
pdf("partial_dependence_plot.pdf", width = 12, height = 18, pointsize = 12)
partial_dependence
dev.off()
...
pdf("partial_dependence_plot_combined.pdf", width = 22, height = 4, pointsize = 12)
partial_dependence
dev.off()
